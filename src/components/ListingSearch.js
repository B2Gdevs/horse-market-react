...
const ListingsSearch = () => {
	...
	const filterSorting = (e) => {
		...
		if (e.target.name === 'distance') {
			let sortedDistanceArray = horsePayload.filter((horse) => horse.distance == e.target.value);
			setHorseData(sortedDistanceArray);
		}
		if (e.target.name === 'age') {
			let sortedDistanceArray = horsePayload.filter((horse) => horse.age == e.target.value);
			setHorseData(sortedDistanceArray);
		}
		if (e.target.name === 'price') {
			let sortedDistanceArray = horsePayload.filter((horse) => horse.price == e.target.value);
			setHorseData(sortedDistanceArray);
		}
		if (e.target.name === 'dateListed') {
			let sortedDistanceArray = horsePayload.filter((horse) => horse.dateListed == e.target.value);
			setHorseData(sortedDistanceArray);
		}
	};
								...
								<div className="filter-item-title">
									<div className="filter-item-wrapper filter-margin">
										...
										<span className="filter-text right-span">
											<i className="fa fa-trash" /> Search
										</span>
									</div>
								</div>
								<SingleSelectFilter
							...
							<div className="p-3">
								...
								<div className="row p-3 sort">
									...
									<div className="mx-2">
										...
										>
											<option selected>Select</option>
											{horseData.map((horse) => <option value={horse.age}>{horse.age}</option>)}
										</select>
									</div>
									<div className="">
										<p>By Price</p>
