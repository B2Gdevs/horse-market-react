...
const BuyListing = (props) => {
	...
	const { value: cardNumber, bind: bindCardNumber, reset: resetCardNumber } = inputHook('');

	const { value: billingAddress, bind: bindBillingAddress, reset: resetBillingAddress } = inputHook('');

	const { value: cardholder, bind: bindCardholder, reset: resetCardholder } = inputHook('');

	const { value: city, bind: bindCity, reset: resetCity } = inputHook('');

	const { value: expiration, bind: bindExpiration, reset: resetExpiration } = inputHook('');

	const { value: zip, bind: bindZip, reset: resetZip } = inputHook('');

	const { value: cvv, bind: bindCVV, reset: resetCVV } = inputHook('');

	const { value: country, bind: bindCountry, reset: resetCountry } = inputHook('');
			...
			<div className="">
				...
				<div className="container p-5">
					...
					<div className="row card p-4 shadow">
						...
						<div className="d-flex">
							<p className={'mr-auto'}>
								Subscription can be cancelled at any time from within your user dashboard
							</p>
						</div>
					</div>
					...
					<div className="row my-5">
						...
						<div className="col-md-12 text-center my-3">
							...
							<Modal title="" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
								<div className="text-center">
									<h5>
										<b>Thanks you for your bussiness!</b>
									</h5>
									<p>
										Your listing has been posted and we hope your experience with us is an enjoyable
										one. You can manage your listings in your dashboard
									</p>
								</div>
							</Modal>
						</div>
